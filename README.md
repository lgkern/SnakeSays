...
That's the load order from the .toc, and two entries depend on it: `Announce.lua`
registers listeners on `Detector.lua` as it loads, and `Radar.lua` builds after
`HUD.lua` so it can anchor itself to the board.
...
position and facing, and faithful secret-value semantics (values that pass around
happily and throw the moment you do arithmetic on them, which is what the guards
in `Position.lua` exist to survive). No game install needed.

```sh
busted
```

`spec/helpers/wow.lua` is the client mock and `spec/helpers/encounter.lua` drives
a scripted pull, placing the player on a bearing, running a wave channel whose
length actually fits the grid, and stepping the replay. Both lint and tests run in
CI on every push.

One rule the suite learned the hard way: **drive the events, don't call the
functions.** A spec that called `Detector.BeginReplay()` directly passed happily
while the replay never started on its own in two of the three modes. Tests here
fire `UNIT_SPELLCAST_CHANNEL_START` and friends and let the addon get itself
there.
